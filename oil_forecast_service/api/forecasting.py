...
def forecast_next_7_days(model=None, lookback: int = 30, horizon: int = 7, device: str = "gpu", show_gui: bool = True):
    ...
    plt.plot([today] + list(dates), [today_price] + list(pred_price), color="#ef2b2d", marker="o", linewidth=2)
    plt.title("오늘 기준 7일 예측 유가")
    plt.ylabel("원/L")
    plt.grid(alpha=0.3)
    plt.xticks(rotation=30, ha="right")
    plt.tight_layout()
    plt.savefig(PATHS.figures / "today_based_forecast.png", dpi=160)
    plt.close()
